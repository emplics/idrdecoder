...
import idr_perm
from idr_perm import pearson_r, pearson_r_loss
...
parser.add_argument('-ist', default=idr_perm.threshold, help='threshold for interacting site (0.0-1.0)')
parser.add_argument('-pgt', default=idr_perm.threshold_pg, help='threshold for protogroup (0.0-1.0)')
args = parser.parse_args()
if args.ist:
  is_threshold = float(args.ist)
#else:
#  is_threshold = idr_perm.threshold 
if args.pgt:
  pg_threshold = float(args.pgt) 
#else:
#  pg_threshold = idr_perm.threshold_pg

la= []
k=c=0
csize=15
para2=idr_perm.para2
...
v3=np.zeros(idr_perm.numpg+1,dtype=float)
...
if os.path.isfile('idr_map_pg_scorefreq_name.pkl'):
  with open('idr_map_pg_scorefreq_name.pkl', 'rb') as f0:
    pgl = pickle.load(f0)
  for i in range(0,idr_perm.numpg):
    v3[i]=float(pgl[i][0])
  mv3=sum(v3)
  for i in range(0,idr_perm.numpg):
...
for record in SeqIO.parse(args.infile,"fasta"):
  ...
  if c==1: 
    ...
    for i in range(0,len(line)):
      for j in range(i,len(line)):
        if j-i<para2 and aa1.index(line[i])<20 and aa1.index(line[j])<20 :
          ...
          n+=1
          for s in range(max(1,j-i-idr_perm.diff_range),min(para2,j-i+idr_perm.diff_range)):
            if s!=(j-i):
              l[aa1.index(line[i])][aa1.index(line[j])][s]+=idr_perm.diff_rate**abs(s-j+i)
      ...
      for i in range(0,20):
        for j in range(0,20):
          for m in range(0,para2-1):
            l[i][j][m]=l[i][j][m]/n
            if m==0 and i==j:
              l[i][j][m]=l[i][j][m]*idr_perm.norm4
    ...
    print('\n',end='')
    ax.add_patch(patches.Circle(xy=(feature[0][(idr_perm.pc1-1)],feature[0][(idr_perm.pc2-1)]),radius=0.005,color='darkred'))
    if idr_perm.showno==1:
       ax.text(feature[0][(idr_perm.pc1-1)],feature[0][(idr_perm.pc2-1)],k+1,ha='left',va='center',fontsize=6,color='darkred')
    if idr_perm.showno==2:
       ax.text(feature[0][(idr_perm.pc1-1)],feature[0][(idr_perm.pc2-1)],name,ha='left',va='center',fontsize=6,color='darkred')
    ...
    for i in range(0,len(line)-1):
      lp0[i]=lp[i]
    for i in range(0,len(line)-1):
      for s in range(max(0,i-idr_perm.diff_range),min(len(line)-1,i+idr_perm.diff_range)):
        lp[s]+=lp0[i]*(idr_perm.diff_rate3**abs(s-i))
    ssm=0.
    for i in range(0,len(line)):
      if (lp[i]*idr_perm.weight)>ssm:
        ssm=lp[i]*idr_perm.weight
    if ssm==0.:
      ssm=1.
    ...
    for i in range(0,len(line)):
      ss=0.
      if max(lp)-min(lp)!=0:
        ss=(((lp[i]*idr_perm.weight)/(ssm))*idr_perm.scorescale) 
        if lp[i]==0:
          ss=0
      if ss<=0:
        print('-',end="")
    ...
    for i in range(0,len(line)):
      if ((lp[lps[i]]*idr_perm.weight)/(ssm))*idr_perm.scorescale < is_threshold:
        break
      print('seq_',k+1,'\t',"islst",'\t',m,'\t',lps[i]+1,line[int(lps[i])],'\t',"{:.3f}".format(((lp[lps[i]]*idr_perm.weight)/(ssm))*idr_perm.scorescale),sep='',end='\n')
      m+=1
    la2=model_1.predict(l)
    mv1=max(sum(la2[0]),1)
    ssm=0.0
    for i in range(0,min(len(la2[0])-1,idr_perm.numpg)):
      if (la2[0][i+1]*idr_perm.weightpg) > ssm:
        ssm=(la2[0][i+1]*idr_perm.weightpg)
    if ssm==0.:
      ssm=1.
    ...
    for i in range(0,min(len(la2[0])-1,idr_perm.numpg)):
      ...
      if max(la2[0])-min(la2[0])!=0:
        ss=(((la2[0][i+1]*idr_perm.weightpg)/(ssm))*idr_perm.scorescale)
        if la2[0][i+1]==0:
          ss=0
      if ss<=0:
        print('-',end="")
    ...
    for i in range(0,min(len(la2[0])-1,idr_perm.numpg)):
      if ((la2[0][la2s[i]]*idr_perm.weightpg)/(ssm))*idr_perm.scorescale < pg_threshold:
        break
      print('seq_',k+1,'\t',"pglst",'\t',m,'\t',pgl[la2s[i]-1][2],'_',la2s[i],'\t',"{:.3f}".format(((la2[0][la2s[i]]*idr_perm.weightpg)/(ssm))*idr_perm.scorescale),'\t',pgl[la2s[i]-1][3],'\t',pgl[la2s[i]-1][4],sep="",end="\n")
      m+=1
    if 0:
